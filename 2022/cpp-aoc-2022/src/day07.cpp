...
namespace Day07 {
    ...
    struct Directory {
        ...
        void traverse_and_put_in_vector(std::vector<Directory*>& out_vec) {
            // depth first FTW
            for (auto& [name, sub_dir] : sub_dirs) {
                sub_dir->traverse_and_put_in_vector(out_vec);
            }
            out_vec.push_back(this);
        }
    };
    ...
    struct Tree {
        ...
        void traverse_and_put_in_vector(std::vector<Directory*>& out_vec) {
            root->traverse_and_put_in_vector(out_vec);
        }
    };
    ...
    int part1(std::istream& input) {
        ...
        tree.traverse_and_put_in_vector(dirs);

        auto large_enough_dirs = dirs
                | std::views::filter([](Directory* dir){ return dir->compute_size() <= 100000; })
                | std::views::transform([](Directory* dir) { return dir->compute_size(); });
                ;

        int total_size{0};
        for (auto size : large_enough_dirs) {
            total_size += size;
        }
